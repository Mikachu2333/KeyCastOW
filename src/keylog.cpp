...
LPCWSTR mouseActions[] = {L"MouseMove",     L"LButtonDown",   L"LButtonUp",
                          L"LButtonDBLCLK", L"RButtonDown",   L"RButtonUp",
                          L"RButtonDBLCLK", L"MButtonDown",   L"MButtonUp",
                          L"MButtonDBLCLK", L"MouseWheel",    L"XButtonDown",
                          L"XButtonUp",     L"XButtonDBLCLK", L"MouseHWheel"};
LPCWSTR mouseClicks[] = {L"LClick", L"RClick", L"MClick"};
LPCWSTR mouseDblClicks[] = {L"LDblClick", L"RDblClick", L"MDblClick"};
    ...
    if (k.vkCode >= 0xA0 && k.vkCode <= 0xA5 ||                  // ctrl / alt
      ...
      if (k.vkCode == VK_BACK || k.vkCode == VK_TAB || k.vkCode == VK_RETURN ||
        ...
        fin = 1;
                       ...
                       GetSymbolFromVK(k.vkCode, k.scanCode, mod, hklLayout))) {
...
LRESULT CALLBACK LLMouseProc(int nCode, WPARAM wp, LPARAM lp) {
  ...
  if (positioning) {
    ...
    if (!(ms->flags & LLMHF_INJECTED)) {
      ...
      if (isAnyModifierDown(modifierState) || modifierState.shift ||
          modifierState.altGr) {
        modifierUsed = TRUE;
        WCHAR modifierkey[64];
        addModifiers(modifierkey, 64, modifierState);
